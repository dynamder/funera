...
//! | [`mod@env`] | Runtime environment: watch-based hot-reload (`FuneraEnv`, `FuneraEnvWatcher`) plus the capability layer (reversible effects + typed services) |
...
//! | [`mod@loader`] | Declarative plugin loader: reconcile a desired plugin set ([`PluginEntry`](loader::PluginEntry)) with hot replacement |
//! | [`mod@middleware`] | Pluggable event interception pipeline (Inspector + Mutator) with typestate error channel |
//! | [`mod@plugin`] | The unified [`Plugin`](plugin::Plugin) abstraction, reactive [`PluginRegistry`](plugin::PluginRegistry), and [`PluginInstance`](plugin::PluginInstance) lifecycle |
